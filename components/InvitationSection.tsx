    ...
    return (
        <section id="invitacion" ref={sectionRef} className="py-20 bg-brand-beige border-t border-brand-gold/10 overflow-hidden relative">
            <div className="max-w-7xl mx-auto px-4 md:px-6">
                ...
                <div className={`relative rounded-2xl overflow-hidden transition-all duration-1000 delay-200 transform ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-20 opacity-0'}`}>
                    <div className="flex justify-center bg-black/5">
                        <div className="relative inline-block max-w-full">
                            <video
                                src="/videoboda.mp4"
                                controls={isPlaying}
                                className="max-w-full max-h-[80vh] w-auto h-auto block shadow-2xl"
                            ...
                            {!isPlaying && showOverlay && (
                                ...
                                >
                                    <div className="animate-pulse-slow mb-4 md:mb-6 group-hover/overlay:scale-110 transition-transform duration-500">
                                        <span className="material-symbols-outlined text-white text-7xl md:text-9xl fill-1 drop-shadow-[0_0_15px_rgba(0,0,0,0.5)]">play_circle</span>
                                    </div>
                                    <div className="text-center px-6 transition-all duration-500">
                                        <p className="text-white text-xl md:text-3xl font-serif italic mb-2 drop-shadow-[0_2px_10px_rgba(0,0,0,0.8)]">"La IA nos dijo que esto sería fácil... claramente mintió"</p>
                                        <p className="text-brand-gold text-xs md:text-sm font-bold uppercase tracking-[0.3em] drop-shadow-[0_1px_5px_rgba(0,0,0,0.8)]">¡Dale al play para ver el desastre!</p>
                                    </div>
                                </div>
                            )}
                            ...
                            {isPlaying && showOverlay && (
                                <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-6 md:p-8 pointer-events-none animate-fade-in">
                                    <div className="text-white">
                                        ...
                                        <p className="text-white/90 text-xs md:text-sm">Nuestros intentos fallidos de crear la invitación perfecta.</p>
                                    </div>
                                </div>
                            )}
                        </div>
