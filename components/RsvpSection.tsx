...
const FORM_CONFIG = {
  RSVP: {
    ...
    fields: {
      name: "entry.1234567890",       // ID del campo Nombre
      guests: "entry.1029384756",     // ID del campo Nº Invitados
      attendance: "entry.5647382910"  // ID del campo Asistencia (Radio buttons)
    }
  },
  ALLERGENS: {
...
const RsvpSection: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>, url: string) => {
    e.preventDefault();
        ...
        <div className="glass-panel p-8 md:p-12 rounded-2xl border border-brand-gold/10 shadow-2xl max-w-2xl mx-auto text-left relative min-h-[400px]">
          ...
          {submissionStatus === 'success' ? (
            <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center animate-fade-in z-20 bg-white/95 rounded-2xl">
              ...
              <h3 className="text-3xl font-serif text-brand-stone mb-2">¡Recibido!</h3>
              <p className="text-brand-text">Muchas gracias por confirmar. <br/>Tus datos se han guardado correctamente en nuestra hoja.</p>
              <button 
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center p-8 text-center animate-fade-in z-20 bg-white/95 rounded-2xl">
              ...
              <h3 className="text-2xl font-serif text-brand-stone mb-2">Algo salió mal</h3>
              <p className="text-brand-text">Por favor, intenta enviarlo de nuevo.</p>
              <button 
                onClick={() => setSubmissionStatus('idle')}
          ...
          <div className={`transition-opacity duration-500 ${submissionStatus !== 'idle' ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
            ...
            {activeTab === 'rsvp' ? (
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                   <label className="cursor-pointer relative flex items-center gap-4 p-4 rounded-lg border border-brand-gold/20 hover:border-brand-gold bg-white transition-all group">
                      <input type="radio" name={FORM_CONFIG.RSVP.fields.attendance} value="Sí, asistiré" className="w-5 h-5 text-brand-gold bg-transparent border-gray-400 focus:ring-brand-gold" defaultChecked />
                      <div>
                   ...
                   <label className="cursor-pointer relative flex items-center gap-4 p-4 rounded-lg border border-brand-gold/20 hover:border-red-400 bg-white transition-all group">
                      <input type="radio" name={FORM_CONFIG.RSVP.fields.attendance} value="No podré asistir" className="w-5 h-5 text-red-500 bg-transparent border-gray-400 focus:ring-red-500" />
                      <div>
              ...
              <form 
                onSubmit={(e) => handleSubmit(e, FORM_CONFIG.ALLERGENS.actionUrl)}
                className="flex flex-col gap-6 animate-fade-in"
              >
                <h3 className="text-2xl font-serif text-brand-stone mb-2">Información de Alérgenos</h3>
                <p className="text-sm text-brand-text -mt-4">Datos importantes para el catering.</p>
          ...
          <div className="mt-8 pt-8 border-t border-brand-gold/10 text-center">
             ...
             <p className="text-brand-text text-sm max-w-lg mx-auto">
               Vuestra presencia es nuestro mayor regalo. No obstante, si queréis tener un detalle con nosotros: <br/>
